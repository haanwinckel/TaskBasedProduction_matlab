function initial_guess = getStartGuess_xT(theta, kappa, z, alphaVec, threshold)
    % getStartGuess_xT generates an initial guess for the optimization problem in prod_fun
...
function xT = adjust_xT(xT, initial_q, H, theta, kappa, z, alphaVec, threshold)
    ...
    while any(imp_l < threshold) && iteration < max_iterations
        try
            % Compute the cumulative sum of exp(xT)
            imp_xT = cumsum(exp(xT));
            imp_q = exp(initial_q);
            
            % Calculate the implied labor demand using the unitInputDemand function
        ...
        % If any labor demand is below the threshold, regenerate xT
    ...
    if iteration == max_iterations
        error('getStartGuess_xT: Could not find suitable initial xT within the maximum iterations. Consider changing tolerance.');
    end
end
